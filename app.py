...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000)    p = CACHE / f"{cache_key(url)}.json"
    if p.exists():
        return json.loads(p.read_text("utf-8"))
    return None
...
# ====================== DỊCH GROK (ĐÃ TĂNG CƯỜNG) ======================
def translate(text, glossary="", custom_prompt=""):
    if not text.strip():
        return "Không có nội dung để dịch."

    print(f"[DEBUG] Bắt đầu dịch - độ dài text: {len(text)} ký tự")

    # Chia theo đoạn (tốt hơn cho tiểu thuyết)
    paragraphs = text.split("\n\n")
    chunks = []
    current = ""
    for p in paragraphs:
...
- Không thêm chú thích, không giải thích, không ghi "Dịch:" hay bất kỳ chữ nào khác.
- Chỉ trả về bản dịch sạch, mượt mà bằng tiếng Việt.
...
=== KẾT THÚC VĂN BẢN ==="""
        ...
        try:
            response = XAI_CLIENT.chat.completions.create(
                model="grok-4.20-non-reasoning",   # model chính xác bạn yêu cầu
                messages=[
                    {"role": "system", "content": "Bạn là dịch giả tiểu thuyết Trung-Việt chuyên nghiệp nhất."},
                    {"role": "user", "content": prompt}
                ],
            ...
            print(f"[DEBUG] Chunk {i+1}/{len(chunks)} dịch thành công - {len(translated_chunk)} ký tự")
            ...
            print(f"[ERROR] Grok chunk {i+1} lỗi: {e}")
            results.append(f"[Lỗi dịch chunk {i+1}: {str(e)}]")

    final = "\n\n".join(results)
    print(f"[DEBUG] Hoàn tất dịch - tổng {len(final)} ký tự")
    return final
