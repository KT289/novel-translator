...
if GEMINI_API_KEY:
    AI_CLIENT = OpenAI(api_key=GEMINI_API_KEY,
                       base_url="https://generativelanguage.googleapis.com/v1beta/openai/")
    MODEL = "gemini-3.1-flash-lite-preview"
    print(f"[OK] Gemini: {MODEL}")
elif XAI_API_KEY:
    AI_CLIENT = OpenAI(api_key=XAI_API_KEY, base_url="https://api.x.ai/v1")
    MODEL = "grok-3-mini"
    print(f"[OK] XAI: {MODEL}")
else:
    print("[WARN] No API key! Set GEMINI_API_KEY or XAI_API_KEY")

CACHE = Path("cache")
CACHE.mkdir(exist_ok=True)

# In-memory store for allhtml data (shared within one worker process)
ALLHTML_MEM = {}
...
def parse_all_html(url):
    ...
    ALLHTML_MEM[url] = result
    set_cache(url, result, prefix="allhtml_")
    print(f"[OK] Parsed {len(chapters)} chapters from {url}")
    return result


def get_chapter_content(novel_url, chapter_index):
    """Get chapter content by index — tries memory, disk, re-parse."""
    # 1. Try in-memory
    data = ALLHTML_MEM.get(novel_url)
    if data:
        chs = data.get("chapters", [])
        if 0 <= chapter_index < len(chs):
            content = chs[chapter_index].get("content", "")
            if content:
                return content

    # 2. Try individual file cache
    vurl = f"{novel_url}#ch_{chapter_index}"
    cached = get_cache(vurl, prefix="raw_")
    if cached:
        return cached

    # 3. Try disk allhtml_ cache
    data = get_cache(novel_url, prefix="allhtml_")
    if data:
        ALLHTML_MEM[novel_url] = data  # restore to memory
        chs = data.get("chapters", [])
        if 0 <= chapter_index < len(chs):
            content = chs[chapter_index].get("content", "")
            if content:
    ...
    # 4. Last resort: re-parse the all.html page
    print(f"[WARN] Cache miss for {novel_url} ch_{chapter_index}, re-parsing...")
    try:
        # Force re-parse by clearing memory cache
        if novel_url in ALLHTML_MEM:
            del ALLHTML_MEM[novel_url]
        data = parse_all_html(novel_url)
        chs = data.get("chapters", [])
        if 0 <= chapter_index < len(chs):
            return chs[chapter_index].get("content", "")
    except Exception as e:
        print(f"[ERR] Re-parse failed: {e}")
...
def extract_memory(cn, vn, mem, url):
    ...
    try:
        r = AI_CLIENT.chat.completions.create(
            model=MODEL,
            messages=[{"role": "user", "content": f"""Trích xuất JSON: "characters"(Trung→Việt), "places", "terms", "summary"(1 câu Việt).
...
def init_memory(url, text):
    ...
    try:
        r = AI_CLIENT.chat.completions.create(
            model=MODEL,
            messages=[{"role": "user", "content": f"""Phân tích chương đầu tiểu thuyết TQ. JSON:
"characters"(Trung→Hán-Việt), "places", "terms", "genre", "summary"(1 câu Việt), "novel_title_vi"
...
def translate(text, glossary="", style="nguyenban", custom_prompt="",
    ...
    if not AI_CLIENT:
        return "[Lỗi] Chưa cấu hình API key. Thêm GEMINI_API_KEY hoặc XAI_API_KEY vào Railway Variables."
    ...
    system_msg = ("Bạn là dịch giả tiểu thuyết Trung-Việt hàng đầu. "
                  "Dịch mượt mà, tự nhiên. Tên/thuật ngữ PHẢI theo bảng thuật ngữ nếu có.")
    results = []
    for i, chunk in enumerate(chunks):
        prompt = f"""Dịch sang tiếng Việt. PHONG CÁCH: {tone}
{mb}
QUY TẮC: Dịch TOÀN BỘ. Tên Hán-Việt nhất quán. Giữ phân đoạn. CHỈ trả bản dịch.
...
def translate_titles(titles, novel_url=""):
    ...
    for start in range(0, len(titles), 80):
        ...
        try:
            r = AI_CLIENT.chat.completions.create(
                model=MODEL,
                messages=[{"role": "user", "content": f"""Dịch tiêu đề chương sang tiếng Việt (Hán-Việt cho tên riêng).
{mem_block}
Trả về ĐÚNG số dòng: số. tiêu đề Việt
...
{numbered}"""}],
            ...
            for line in raw.split("\n"):
                ...
                if line and line[0].isdigit():
                    parts = line.split(".", 1)
                    all_vn.append(parts[1].strip() if len(parts) > 1 else line)
                elif line:
                    all_vn.append(line)
        except Exception:
            all_vn.extend(batch)
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "model": MODEL or "NONE",
                    "api_key": "set" if AI_CLIENT else "MISSING"})
...
def api_chapters():
    ...
    try:
        if is_all_page(url):
            ...
            # ALSO write individual cache files as backup
            for i, ch in enumerate(chs):
                vurl = f"{url}#ch_{i}"
                set_cache(vurl, ch["content"], prefix="raw_")

            ch_list = [{"title": ch["title"], "url": f"{url}#ch_{i}"}
                       for i, ch in enumerate(chs)]
            return jsonify({"chapters": ch_list, "novel_title": data.get("title", ""),
            ...
            title = ""
            try:
                t = fetch(url).find("title")
                if t:
                    title = t.get_text(strip=True).split("_")[0].split("-")[0].strip()
            except Exception:
                pass
            return jsonify({"chapters": chs, "novel_title": title,
                            "mode": "standard", "novel_url": url})
    except Exception as e:
...
def api_init_memory():
    ...
    if is_all_page(novel_url):
        text = get_chapter_content(novel_url, 0) or ""
    else:
        fu = d.get("first_chapter_url", "")
        if fu:
            try:
                text = get_content_standard(fu)
            except Exception:
                pass
...
def api_translate():
    ...
    # For all.html virtual URLs: get content from allhtml data
    if "#ch_" in cu:
        try:
            idx = int(cu.split("#ch_")[1])
            base_url = cu.split("#ch_")[0]
            # Try the robust getter (memory → individual cache → disk allhtml → re-parse)
            raw = get_chapter_content(base_url, idx)
        except (ValueError, IndexError) as e:
            print(f"[ERR] Chapter index parse error: {e}")

    # For standard URLs: try cache then fetch
    if not raw:
    ...
    if not raw:
        return jsonify({"error": "Không tìm thấy nội dung chương. Thử tải lại trang."}), 500
...
def _ms(m):
    return {"characters": len(m.get("characters", {})),
